...
# -*- coding: utf-8 -*-
"""
test_operational_hardening.py - Phase 8 item G: operational
hardening.
...
- **Timeout behavior / child-process cleanup for bounded subprocesses**:
  ...
  ``test_serial_parallel_equivalence.py``'s own module docstring
...
from tests.cpp_parity_live._driver_support import run_phase8_batch
...
#: The OTHER 6 core-unit test modules formerly grouped as "Phase 8", used
#: for the order-independence and no-checkout-debris checks below.
#: Deliberately EXCLUDES this very file (``test_operational_hardening.py``)
#: - including it would make these two tests spawn a subprocess that
#: re-runs this same test, which would spawn another subprocess that
#: re-runs it again, unboundedly (a real bug caught during authoring,
#: before it could hang the suite; the excluded module's own tests are
#: still fully exercised whenever the real, non-recursive test run
#: collects and executes this file directly).
_SIBLING_UNIT_TEST_MODULE_PATHS = [
...
def test_bounded_representative_burnup_call_completes_within_a_generous_measured_bound():
    ...
    **Correction pass (2026-09-11), item 5**: this now runs the call
    through :func:`~tests.cpp_parity_live._driver_support.
...
def test_no_checkout_local_generated_debris_check_rejects_a_failing_run_while_still_checking_cleanup(monkeypatch):
    ...
    subprocess fails."""
    import tests.unit.test_operational_hardening as this_module
...
def test_phase8_modules_create_no_checkout_local_generated_debris():
    ...
    _assert_no_debris_and_clean_pytest_run(_SIBLING_UNIT_TEST_MODULE_PATHS, timeout=180.0)
...
def test_phase8_modules_pass_in_both_forward_and_reversed_file_order():
    ...
    **Correction pass (2026-09-11), item 4**: both runs now go through
    ...
    forward = _assert_no_debris_and_clean_pytest_run(_SIBLING_UNIT_TEST_MODULE_PATHS, timeout=180.0)
    reversed_result = _assert_no_debris_and_clean_pytest_run(
        list(reversed(_SIBLING_UNIT_TEST_MODULE_PATHS)), timeout=180.0,
    )
...
def test_repeated_serial_burnup_calls_are_deterministic():
    ...
    separately in ``test_serial_parallel_equivalence.py``."""
