...
# -*- coding: utf-8 -*-
"""
test_default_emissions_equivalence.py - Phase 6 investigation A:
...
like ``test_emissions_parity.py``'s ``legacy``/``expanded`` tests.
...
2. **The faithful full-pipeline C++ configuration is demonstrated to apply
   ...
   (``consume_emissions_equivalence.default-equiv-g*``, ``rtol=5e-07``), never invented.
...
from tests.cpp_parity_live._emissions_equivalence_contract import golden_rows_by_case, require_golden_tree
...
#: literal - identical to ``test_emissions_parity.py``'s own constant.
...
#: parity (``consume_expanded_matrix.emissions_legacy``/``emissions_expanded``), reused
