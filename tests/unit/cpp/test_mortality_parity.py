...
# -*- coding: utf-8 -*-
"""
test_mortality_parity.py - Phase 4 coverage for ``mort_bolchar``,
...
#: centralized policy - ``mortality_expanded_matrix.BolCha``, ``.CroDam`` and ``.CroSco``
...
ATOL_PROB = expanded_matrix_tolerance("mortality", "BolCha")[0]
...
def test_crnsch_probability_matches_cpp(case_id, request):
    ...
    pinned revision) via
    :data:`~tests.cpp_parity_live._expanded_matrix_contract.CROSCO_BARK_SOURCE`, so no
