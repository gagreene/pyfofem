...
# -*- coding: utf-8 -*-
"""
test_tree_structure_parity.py - Phase 4 coverage for
...
``test_consumption_parity.py``. The two are never conflated here.
...
#: from the single centralized ``canopy_cover_expanded_matrix.all`` policy entry rather
...
#: is 4.54e-05.
_CANOPY_COVER_ATOL, RTOL_AREA = expanded_matrix_tolerance("canopy_cover", "all")
ATOL_AREA = _CANOPY_COVER_ATOL

#: Absolute tolerance for a bark-thickness value (inches), retrieved from
#: the centralized ``bark_thick_expanded_matrix.all`` policy entry - the golden's own
...
ATOL_BARK_THICK = expanded_matrix_tolerance("bark_thick", "all")[0]
