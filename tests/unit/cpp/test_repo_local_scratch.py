...
# -*- coding: utf-8 -*-
"""
test_repo_local_scratch.py - Phase 7 correction pass item 4: contract
tests for ``tests/cpp_parity_live/_scratch.py``, the repository-local
scratch-directory utility every dataset's generator/test now uses instead
...
def _assert_no_residue_after_module():
    ...
    2): after every test in this module has run, :data:`SCRATCH_ROOT`
    ...
    if os.path.isdir(SCRATCH_ROOT):
        leftover = os.listdir(SCRATCH_ROOT)
        assert not leftover, (
            "SCRATCH_ROOT contains leftover entries after "
            f"test_repo_local_scratch.py's own tests ran: {leftover}"
        )
...
def _unique_subdir(label: str) -> str:
    ...
    scratch subtree (which legitimately keeps :data:`SCRATCH_ROOT`
...
def test_scratch_root_is_under_project_root():
    """:data:`SCRATCH_ROOT` itself must resolve under
    ``PROJECT_ROOT``."""
    resolved = os.path.realpath(SCRATCH_ROOT)
...
def test_scratch_root_creates_and_returns_a_repo_local_directory():
    ...
    existing directory under :data:`SCRATCH_ROOT`."""
    created = scratch_root(_unique_subdir("test_scratch_probe"))
    try:
...
def test_scratch_tempdir_does_not_remove_a_parent_still_owned_by_another_active_context():
    ...
    parent = os.path.join(SCRATCH_ROOT, subdir)
...
def test_scratch_tempdir_pruning_never_removes_scratch_root_itself():
    """Even when a caller's own scratch subdirectory chain fully empties
    out, :data:`SCRATCH_ROOT` itself must never be pruned away -
...
def test_verify_under_project_root_rejects_a_path_whose_resolved_form_escapes_project_root(monkeypatch):
    """A path that LOOKS like it is under :data:`SCRATCH_ROOT`
    ...
    the complete audited file surface forbids
    (``test_burnup_extended_skip_hygiene.py`` asserts zero
    ...
    outside_target = os.path.join(
        ...
        "scratch-escape-target-never-created",
    )
    escape_candidate = os.path.join(SCRATCH_ROOT, "looks_like_a_real_subdir")
