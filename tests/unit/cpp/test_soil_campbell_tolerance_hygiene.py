...
# -*- coding: utf-8 -*-
"""
test_soil_campbell_tolerance_hygiene.py - Correction-pass part-2 item-3
meta-test: prevents
``tests/unit/cpp/test_soil_campbell_characterization.py`` from regressing
back to a raw, hardcoded ``pytest.approx(..., abs=<literal>)``
...
precision (see ``_soil_campbell_contract.py``'s own docstring for that constant).

Scope is deliberately narrow: ONLY the characterization module, not its
companion ``test_soil_campbell_contract.py``. That companion module's
...
mirror ``test_expanded_matrix_xfail_hygiene.py``'s established pattern: every
...
from tests.cpp_parity_live._soil_campbell_contract import CHARACTERIZATION_REGRESSION_PRECISION_DEGC

_CHARACTERIZATION_MODULE_PATH = os.path.join(
    PROJECT_ROOT, "tests", "unit", "cpp", "test_soil_campbell_characterization.py"
)
...
def test_characterization_module_declares_and_imports_the_named_constant():
    ...
    assert "CHARACTERIZATION_REGRESSION_PRECISION_DEGC" in imported_names, (
        "test_soil_campbell_characterization.py no longer imports "
        "CHARACTERIZATION_REGRESSION_PRECISION_DEGC from _soil_campbell_contract.py"
    )
...
def test_characterization_module_has_no_imperative_pytest_xfail_call():
    ...
    ``test_expanded_matrix_xfail_hygiene.py``'s identical rule."""
    tree = _parse_characterization_module()
    offending = []
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
