...
# -*- coding: utf-8 -*-
"""
test_consumption_parity.py - Phase 4 Tier-2 consumption coverage for
...
value is read from the committed, fully manifested expanded_matrix golden dataset
under ``tests/test_data/test_golden_output/expanded_matrix/``, which was produced by
...
# Fail CLOSED, not open: a missing/incomplete committed expanded_matrix golden
...
#: centralized policy (never a literal here) - ``consume_expanded_matrix.duff_percent``,
#: ``consume_expanded_matrix.mineral_soil``, ``consume_expanded_matrix.duff_depth`` and
#: ``consume_expanded_matrix.herb`` all record the SAME 1e-05, because it is not a
#: per-route tuned value: every expanded_matrix golden's scientific columns are
...
ATOL_PERCENT = expanded_matrix_tolerance("consume", "mineral_soil")[0]

#: Absolute tolerance for a T/ac or inch-valued consume output, likewise
#: retrieved from the policy - ``consume_expanded_matrix.herb`` records the same 1e-05
#: output-resolution bound as ``consume_expanded_matrix.duff_depth``,
#: ``consume_expanded_matrix.litter_cp`` and ``shrub_herb_eq_expanded_matrix.*``. Measured maximum
...
#: resolution.
ATOL_LOAD = expanded_matrix_tolerance("consume", "herb")[0]
...
def test_consume_scenarios_all_produced_an_ok_oracle_row():
    """Every expanded_matrix consume scenario must have produced a real, successful
...
@pytest.mark.parametrize("mode", EXPANDED_MATRIX_MODES)
def test_every_expanded_matrix_manifest_is_structurally_valid(mode):
    """Each committed expanded_matrix manifest must pass the shared validator's
    structural, exact-value and internal-consistency checks."""
    manifest = golden_manifest(mode)
    assert manifest is not None, f"no committed expanded_matrix manifest for {mode!r}"
    assert manifest.get("dataset") == "expanded_matrix", (
        f"{mode}: manifest must record dataset='expanded_matrix', got "
...
def test_shrub_herb_scenarios_all_produced_an_ok_oracle_row():
    """Every expanded_matrix shrub_herb_eq scenario must have produced a successful
    C++ row."""
