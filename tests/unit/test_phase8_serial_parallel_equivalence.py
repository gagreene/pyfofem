...
# -*- coding: utf-8 -*-
"""
test_phase8_serial_parallel_equivalence.py - Phase 8 item B: serial/
...
exits 0 every time - see F-64 in ``gate0/04-findings.md`` for the full
reconciliation). Every ``num_workers>1`` call in this module now goes
through ``tests.cpp_parity_live._phase8_driver_support.
run_phase8_batch()``/``invoke_phase8_driver()``, which spawn the real,
installed ``pyfofem`` in a PLAIN ``python`` subprocess
(``tests/cpp_parity_live/phase8_parallel_driver.py`` - deliberately not
...
_phase8_driver_support.run_phase8_batch`, i.e. BOTH run inside the same
...
exact, fully-enforced schema (see ``_phase8_driver_support.py``'s own
...
``_phase8_driver_support.py``'s own docstring for the corrected
...
from tests.cpp_parity_live._phase8_driver_support import (
    DRIVER_PATH,
...
#: The complete Phase 8 test-module surface this module's own hygiene
#: check audits for direct, unbounded ``num_workers>1`` calls.
_PHASE8_TEST_MODULE_PATHS = (
...
def test_bounded_driver_accepts_ok_false_with_returncode_exactly_1(monkeypatch):
    ...
    import tests.cpp_parity_live._phase8_driver_support as support
...
def test_no_direct_num_workers_gt1_calls_in_any_collected_phase8_module():
    ...
    all_findings = []
    for path in _PHASE8_TEST_MODULE_PATHS:
