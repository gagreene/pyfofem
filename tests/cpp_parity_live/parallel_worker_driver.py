...
# -*- coding: utf-8 -*-
"""
parallel_worker_driver.py - Non-collected driver process that calls the
...
This is a benign pytest-``faulthandler``-only dump reproducibly triggered
by the first-ever ``num_workers>1`` test coverage - no process crash or
wrong result - see ``gate0/04-findings.md`` for the full reconciliation,
including what is and is not proven about the underlying condition itself.
...
``tests.cpp_parity_live._driver_support.run_phase8_batch()``,
...
(``_driver_support.run_phase8_batch()``) treats a missing/
...
``tests/unit/test_serial_parallel_equivalence.py`` and
``tests/unit/test_operational_hardening.py`` to exercise the
...
_driver_support.invoke_phase8_driver()`` validates every one of
