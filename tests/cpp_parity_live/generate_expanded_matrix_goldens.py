...
# -*- coding: utf-8 -*-
"""
generate_expanded_matrix_goldens.py - Generate the Phase 4 Tier-2 scenario-matrix
golden dataset (many rows per mode) with a full provenance manifest.

Phase 4 adds **no harness mode**. Every scenario in
``_expanded_matrix_contract.py`` is driven through one of the six already-qualified
...
The canonical golden tree is never read, written, or promoted by this
module: ``GOLDEN_ROOT`` here is
``tests/test_data/test_golden_output/expanded_matrix/``, a sibling of
``canonical/``.
...
Usage:
    python tests/cpp_parity_live/generate_expanded_matrix_goldens.py
    python tests/cpp_parity_live/generate_expanded_matrix_goldens.py --verify-only
...
def _generate_one(mode: str, out_dir: str) -> str:
    ...
    result = run_harness(
        mode, spec["header"], expanded_matrix_rows(mode), prefix,
        output_suffixes=spec["suffixes"], **species_kw,
    )
    if result.returncode != 0:
        raise RuntimeError(
            f"expanded_matrix golden generation for mode={mode!r} failed "
...
def generate_all(out_root: str, *, qualify: bool = True) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_root:
        ...
        _promote(tmp_root, out_root, list(EXPANDED_MATRIX_MODES))
...
def main() -> int:
    ...
    if args.verify_only:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            mismatches = verify_regeneration(
                GOLDEN_ROOT, tmp, list(EXPANDED_MATRIX_MODES)
            )
            if mismatches:
                print("EXPANDED_MATRIX DETERMINISM CHECK FAILED:")
                for message in mismatches:
                    print(" -", message)
                return 1
            print(
                "EXPANDED_MATRIX DETERMINISM CHECK PASSED: fresh regeneration is "
    ...
    print(f"Generated expanded_matrix goldens under {GOLDEN_ROOT}")
    return 0
