...
# -*- coding: utf-8 -*-
"""
_driver_support.py - Invocation/validation helpers for
``parallel_worker_driver.py`` (Phase 8 correction pass item 1).
...
See ``parallel_worker_driver.py``'s own module docstring for why.
...
``test_serial_parallel_equivalence.py`` already does.

**Correction pass (2026-09-11, third round) - the error-path exit code is
now enforced EXACTLY, not merely "nonzero".** ``parallel_worker_driver.py``
...
DRIVER_PATH = os.path.join(PROJECT_ROOT, "tests", "cpp_parity_live", "parallel_worker_driver.py")

#: Default bound for a driver invocation - generous relative to the
#: ~0.04s measured cost of a small serial burnup call (see
#: ``test_operational_hardening.py``), so a real hang is still
...
def invoke_phase8_driver(
    ...
    _assert_no_native_crash_indicators(result.stderr, context="parallel_worker_driver.py")
    assert result.stderr == "", (
        "parallel_worker_driver.py: stderr must be exactly empty (documented "
