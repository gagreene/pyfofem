...
# -*- coding: utf-8 -*-
"""
generate_phase7_goldens.py - Generate the Phase 7 item E additional-
...
``_phase7_contract.py`` is driven through the already-qualified
...
``generate_phase4_goldens.py``/``generate_phase5_goldens.py``/
``generate_phase6_goldens.py`` do:
...
The Phase 2/4/5/6 golden trees are never read, written, or promoted by this
module: ``GOLDEN_ROOT`` here is
``tests/test_data/test_golden_output/phase7/``, a sibling of
``phase2/``/``phase4/``/``phase5/``/``phase6/``.

Usage:
    python tests/cpp_parity_live/generate_phase7_goldens.py
    python tests/cpp_parity_live/generate_phase7_goldens.py --verify-only
...
def generate_all(out_root: str, *, qualify: bool = True) -> None:
    ...
    with scratch_tempdir("generate", prefix="generate-all") as tmp_root:
        ...
        _promote(tmp_root, out_root, list(PHASE7_MODES))
...
def main() -> int:
    ...
    if args.verify_only:
        with scratch_tempdir("verify-only", prefix="verify") as tmp:
            ...
            mismatches = verify_regeneration(
                GOLDEN_ROOT, tmp, list(PHASE7_MODES)
            )
            if mismatches:
                print("PHASE 7 DETERMINISM CHECK FAILED:")
                for message in mismatches:
                    print(" -", message)
                return 1
            print(
                "PHASE 7 DETERMINISM CHECK PASSED: fresh regeneration is "
                "byte-identical (SHA-256 compared) to the committed Phase 7 "
    ...
    print(f"Generated Phase 7 goldens under {GOLDEN_ROOT}")
    return 0
