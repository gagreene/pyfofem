...
# -*- coding: utf-8 -*-
"""
generate_emissions_equivalence_goldens.py - Generate the Phase 6 investigation A
...
``_emissions_equivalence_contract.py`` is driven through the already-qualified ``consume``
...
``generate_expanded_matrix_goldens.py``/``generate_soil_campbell_goldens.py`` do:
...
The canonical/expanded_matrix/soil_campbell golden trees are never read,
written, or promoted by this module: ``GOLDEN_ROOT`` here is
``tests/test_data/test_golden_output/emissions_equivalence/``, a sibling of
``canonical/``/``expanded_matrix/``/``soil_campbell/``.

Usage:
    python tests/cpp_parity_live/generate_emissions_equivalence_goldens.py
    python tests/cpp_parity_live/generate_emissions_equivalence_goldens.py --verify-only
...
GOLDEN_TOLERANCE_KEYS = {
    ...
    for mode in EMISSIONS_EQUIVALENCE_MODES
}
...
def _generate_one(mode: str, out_dir: str) -> str:
    ...
    """
    if mode not in EMISSIONS_EQUIVALENCE_MODES:
        raise KeyError(f"unknown Phase 6 mode: {mode!r}")
    os.makedirs(out_dir, exist_ok=True)
    prefix = os.path.join(out_dir, mode)
    header = MODES[mode]["header"]
    result = run_harness(
        mode, header, emissions_equivalence_rows(mode), prefix,
        output_suffixes=MODE_OUTPUT_SUFFIXES[mode],
    )
    if result.returncode != 0:
        raise RuntimeError(
            f"emissions_equivalence golden generation for mode={mode!r} failed "
...
def generate_all(out_root: str, *, qualify: bool = True) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_root:
        ...
        _promote(tmp_root, out_root, list(EMISSIONS_EQUIVALENCE_MODES))
...
def main() -> int:
    ...
    if args.verify_only:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            mismatches = verify_regeneration(
                GOLDEN_ROOT, tmp, list(EMISSIONS_EQUIVALENCE_MODES)
            )
            if mismatches:
                print("EMISSIONS_EQUIVALENCE DETERMINISM CHECK FAILED:")
                for message in mismatches:
                    print(" -", message)
                return 1
            print(
                "EMISSIONS_EQUIVALENCE DETERMINISM CHECK PASSED: fresh regeneration is "
                "byte-identical (SHA-256 compared) to the committed emissions_equivalence "
    ...
    print(f"Generated emissions_equivalence goldens under {GOLDEN_ROOT}")
    return 0
