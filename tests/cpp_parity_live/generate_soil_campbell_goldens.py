...
# -*- coding: utf-8 -*-
"""
generate_soil_campbell_goldens.py - Generate the Phase 5 ``soil_campbell``
...
``generate_expanded_matrix_goldens.py`` does:
...
The canonical/expanded_matrix golden trees are never read, written, or
promoted by this module: ``GOLDEN_ROOT`` here is
``tests/test_data/test_golden_output/soil_campbell/``, a sibling of
``canonical/``/``expanded_matrix/``.
...
(``tests/unit/cpp/test_soil_campbell_characterization.py``), but it is
...
``tolerance_policy.json``'s ``soil_campbell`` section records ``duff``/
...
``_soil_campbell_contract.py``'s module docstring for the full evidence chain.
...
is tracked as its own ``soil_campbell.duff_moisture_unit`` route,
...
Usage:
    python tests/cpp_parity_live/generate_soil_campbell_goldens.py
    python tests/cpp_parity_live/generate_soil_campbell_goldens.py --verify-only
...
GOLDEN_TOLERANCE_KEYS = {
    ...
    for mode in SOIL_CAMPBELL_MODES
}
...
def _generate_one(mode: str, out_dir: str) -> str:
    ...
    **Why no ``side_file_sha256`` manifest entry.** Unlike Phase 2/4's
    ...
    so :func:`~tests.cpp_parity_live.generate_canonical_goldens.verify_regeneration`'s
    ...
    """
    if mode not in SOIL_CAMPBELL_MODES:
    ...
    try:
        ...
        result = run_harness(
            mode, SOIL_CAMPBELL_HEADER, soil_campbell_rows(mode), prefix,
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"soil_campbell golden generation for mode={mode!r} failed "
...
def generate_all(out_root: str, *, qualify: bool = True) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_root:
        ...
        _promote(tmp_root, out_root, list(SOIL_CAMPBELL_MODES))
...
def main() -> int:
    ...
    if args.verify_only:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            mismatches = verify_regeneration(
                GOLDEN_ROOT, tmp, list(SOIL_CAMPBELL_MODES)
            )
            if mismatches:
                print("SOIL_CAMPBELL DETERMINISM CHECK FAILED:")
                for message in mismatches:
                    print(" -", message)
                return 1
            print(
                "SOIL_CAMPBELL DETERMINISM CHECK PASSED: fresh regeneration is "
    ...
    print(f"Generated soil_campbell goldens under {GOLDEN_ROOT}")
    return 0
