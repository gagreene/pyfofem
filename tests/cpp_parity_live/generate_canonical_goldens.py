...
# -*- coding: utf-8 -*-
"""
generate_canonical_goldens.py - Generate the minimal "canonical" (Phase 2)
qualifying golden dataset (one per mode) with a full provenance manifest.
...
- Generation writes into a fresh temporary directory; the committed
  ``tests/test_data/test_golden_output/canonical/`` tree is only replaced as
...
Usage:
    python tests/cpp_parity_live/generate_canonical_goldens.py
    python tests/cpp_parity_live/generate_canonical_goldens.py --verify-only
...
GOLDEN_ROOT = os.path.join(TEST_GOLDEN_DIR, "canonical")
...
def verify_regeneration(committed_root: str, fresh_root: str, modes) -> List[str]:
    ...
    *fresh_root* against the committed tree in *committed_root* — the SAME
    ...
    ``test_generate_canonical_goldens.py``, so a corrupted/missing/extra/
...
def main() -> int:
    ...
    print(f"Generated canonical goldens under {GOLDEN_ROOT}")
    return 0
